...
export function buildChatBubbles({scene,camera,getAnchor,onSound}){
  ...
  function show(user,text){
    ...
    if(!b){
      ...
      b={canvas,texture,sprite};bubbles.set(user,b);
    }
    Object.assign(b,{text:Array.from(text),born:performance.now(),last:-1,sounded:false});
  }
  function paint(b,n){
  ...
  function update(){
    ...
    for(const [user,b] of bubbles){
      if(!b.text)continue;
      const anchor=getAnchor(user),age=now-b.born,typing=reduced.matches?0:b.text.length*28,end=typing+5500;
      ...
      b.sprite.position.y+=(Math.abs(anchor.rotation.x)>1?1.1:2.65)+b.sprite.scale.y/2;
      ...
      const n=reduced.matches?b.text.length:Math.min(b.text.length,Math.floor(age/28));
