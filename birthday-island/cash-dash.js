...
export function buildCashDash({scene,camera,ground,resolveSite,rewards,online,male,roleUI=false,getNetwork,isPlaying,candlesBlown,getCollector,onPickup,onStart,onCue=()=>{}}){
  ...
  // Enclose the whole bundle, including its .298-deep gold ribbon.
  // Write shell depth too: the ocean renders later and would otherwise overwrite
  // outline pixels even behind the pickup. Foreground scenery still occludes it.
  const glow=new THREE.InstancedMesh(new THREE.BoxGeometry(1.07,.58,.34),new THREE.MeshBasicMaterial({color:new THREE.Color(1.15,1.15,1.15),side:THREE.BackSide,depthWrite:true,toneMapped:false}),sites.length);
  ...
  const button=document.createElement('button');button.className='cash-dash-start';button.textContent=`Start ${DASH_NAME} · G`;button.hidden=true;card.append(button);
  let lastBeat=null,bonus=0;
  let dash=null,serverAnchor=0,localAnchor=0,previousPhase='idle',lastCount=0,feedback=null,lastFeedback=-Infinity;
  ...
  function accept(next,{silent=false}={}){
    ...
    if(!dash)return;
    if(!old||old.startAt!==dash.startAt){pending.clear();lastBeat=null;lastCount=0;feedback=null;lastFeedback=-Infinity;rewards.restore(bonus,0);onStart();}
    const count=dash.collected.length;
    if(count>lastCount){
      const index=dash.collected[count-1];
      if(silent){feedback=null;rewards.restore(count*CASH_VALUE+bonus,count);}
      else{
  ...
  function flushFeedback(time,force=false){
    ...
    rewards.collectInstant({amount,total:count*CASH_VALUE+bonus,packages:count,x:visible?(projected.x*.5+.5)*innerWidth:innerWidth*.5,y:visible?(-projected.y*.5+.5)*innerHeight:innerHeight*.55});
  ...
  return {
    start,
    get winnings(){return (dash?.collected.length||0)*CASH_VALUE+bonus;},
    awardBonus(){if(bonus)return;bonus=50;rewards.collectInstant({amount:50,total:this.winnings,packages:dash?.collected.length||0,x:innerWidth/2,y:innerHeight/2});},
    // True once a dash has run to its end (time up or every bundle found).
    get finished(){return dashPhase(dash,now())==='finished';},
    sync(snapshot){if(snapshot.world.hoopPrize&&!bonus){bonus=50;rewards.restore((snapshot.world.cashDash?.collected.length||0)*CASH_VALUE+bonus);}serverAnchor=snapshot.serverTime;localAnchor=performance.now();accept(snapshot.world.cashDash,{silent:!!snapshot.welcome});},
    update(){
      ...
      button.hidden=roleUI?!male||!!dash:online?!(male&&candlesBlown()&&!dash):!!dash&&phase!=='finished';
      ...
      button.textContent=online||roleUI?`Start ${DASH_NAME} · G`:`Try ${DASH_NAME} again · G`;
      timer.hidden=phase!=='countdown'&&phase!=='running';
      const go=phase==='running'&&time-dash.startAt<900;
      timer.setAttribute('data-phase',go?'go':phase);
      ...
      else if(phase==='running'){label.textContent=go?'Collect as many as you can!':'';clock.textContent=go?'GO!':Math.ceil((dash.endAt-time)/1000)+'s';}
      const beat=phase==='countdown'?Math.ceil((dash.startAt-time)/1000):go?'go':null;
      if(beat!==null&&beat!==lastBeat){onCue(beat);lastBeat=beat;}
      if(phase!==previousPhase){
        if(phase==='finished'&&previousPhase==='running'){flushFeedback(time,true);rewards.celebrate?.(collected.size*CASH_VALUE+bonus);onCue('finish');}
        announcement.textContent=phase==='countdown'?`${COIN_MODE?'Coin':'Cash'} dash starts in three seconds.`:phase==='running'?`Go! Walk into ${COIN_MODE?'coins':'cash bundles'}. Sixty seconds.`:phase==='finished'?`${COIN_MODE?'Coin':'Cash'} dash complete. ${formatCash(collected.size*CASH_VALUE+bonus)}${COIN_MODE?' coins':''} collected.`:'';
        previousPhase=phase;
      }
      ...
      for(const site of sites){
        ...
        // YXZ applies the world-up spin outside the bundle's portrait tilt.
        transform.rotation.set(0,reduced.matches?.4:(time-dash.startAt)/1000*1.35+i,-.18,'YXZ');transform.quaternion.multiply(portrait);
        const entrance=Math.max(0,Math.min(1,(time-(dash.startAt-3000)-i*38)/450));
        const reveal=reduced.matches?1:1-Math.pow(1-entrance,3);
        transform.position.y-=(1-reveal)*.7;
        transform.scale.setScalar(collected.has(i)?0:.72*reveal);transform.updateMatrix();mesh.setMatrixAt(i,transform.matrix);
