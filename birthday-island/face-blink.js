...
import {expressionIndex,EXPRESSION_MS} from './expressions.js?v=kiss-4';
...
export function addFaceBlink(mesh,male){
  ...
  mesh.material.onBeforeCompile=shader=>{
    ...
    // Blinks replace the eye band; expressions also adjust the printed mouth.
    shader.fragmentShader=shader.fragmentShader.replace('#include <map_fragment>',`
      ...
      if(mouthBand&&uExpression==1){
        ...
        vec3 ink=mix(vec3(${male?'0.20,0.065,0.045':'0.42,0.045,0.07'}),vec3(0.055,0.018,0.018),crease);
  ...
  mesh.material.customProgramCacheKey=()=>`face-expressions-${male?'male':'female'}-kiss-3`;
